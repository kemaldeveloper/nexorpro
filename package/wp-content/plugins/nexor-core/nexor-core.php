...
final class Nexor_Core {
	...
	public static function calculate( WP_REST_Request $request ): WP_REST_Response|WP_Error {
		$valid = self::validate_request( $request ); if ( is_wp_error( $valid ) ) return $valid;
		$a = (array) $request->get_json_params(); $s = self::settings();
		$allowed = array(
		...
		$min = max( (float)$s['min_budget'], $min ); $width = ( $max - $min ) / $min;
		if ( $width < .15 ) $max = $round( $min * 1.20 ); elseif ( $width > .25 ) $max = $round( $min * 1.25 ); if ( $max <= $min ) $max = $round( $min * 1.25 );
	...
	public static function project_box( WP_Post $post ): void {
		...
		foreach ( $fields as $key=>$label ) { $value=get_post_meta($post->ID,'_nexor_'.$key,true); printf('<p><label><strong>%s</strong><br><textarea name="nexor_%s" rows="%s" style="width:100%%">%s</textarea></label></p>',esc_html($label),esc_attr($key),in_array($key,array('task','result'),true)?4:2,esc_textarea($value)); }
	...
	public static function lead_box( WP_Post $post ): void {
		$data=(array)get_post_meta($post->ID,'_nexor_lead_data',true); echo '<table class="widefat striped"><tbody>'; foreach($data as $key=>$value)printf('<tr><th>%s</th><td>%s</td></tr>',esc_html($key),nl2br(esc_html((string)$value))); echo '</tbody></table>';
		$status=get_post_meta($post->ID,'_nexor_lead_status',true); echo '<p><strong>Статус:</strong> '.esc_html($status).'</p>'; $url=wp_nonce_url(admin_url('admin-post.php?action=nexor_resend_lead&lead_id='.$post->ID),'nexor_resend_'.$post->ID); echo '<p><a class="button button-primary" href="'.esc_url($url).'">Повторно отправить в Telegram</a></p>';
	}
	...
	public static function resend_lead(): void {
		$id=absint($_GET['lead_id']??0); if(!$id||!current_user_can('edit_post',$id))wp_die('Недостаточно прав.'); check_admin_referer('nexor_resend_'.$id); $sent=self::send_telegram($id); update_post_meta($id,'_nexor_lead_status',is_wp_error($sent)?'telegram_error':'telegram_sent'); wp_safe_redirect(get_edit_post_link($id,'url')); exit;
	}
	...
	public static function sitemap_query(array $args,string $post_type):array { if('page'===$post_type){$thank=get_page_by_path('thank-you');if($thank)$args['post__not_in']=array($thank->ID);}return $args; }
	public static function robots(string $output,bool $public):string { return str_contains($output,'wp-sitemap.xml') ? $output : $output."\nSitemap: ".home_url('/wp-sitemap.xml')."\n"; }
	public static function lead_columns(array $cols):array { $cols['nexor_status']='Статус';$cols['nexor_phone']='Телефон';return $cols; }
	public static function lead_column(string $col,int $id):void { if('nexor_status'===$col)echo esc_html(get_post_meta($id,'_nexor_lead_status',true));if('nexor_phone'===$col){$d=(array)get_post_meta($id,'_nexor_lead_data',true);echo esc_html($d['phone']??'');} }
	public static function content_replacements(array $r):array { $s=self::settings();return array_merge($r,array('+7 (926) 083-23-24'=>$s['phone_display'],'+79260832324'=>preg_replace('/\D/','',$s['phone_link'])?'+'.preg_replace('/\D/','',$s['phone_link']):$s['phone_link'],'nexor.msk@mail.ru'=>$s['email'],'Ежедневно с 9:00 до 21:00'=>$s['hours'],'Москва и Московская область'=>$s['region'],'https://t.me/nexor_msk'=>$s['telegram_url'],'https://vk.com/club238015413'=>$s['vk_url'])); }
	public static function legal_replacements(string $content):string { $s=self::settings(); return str_replace(array('352803113189','324350000048081'),array($s['inn'],$s['ogrnip']),$content); }
	...
	public static function seed_content(): void {
		$dir=get_theme_file_path('content');$meta_file=$dir.'/metadata.json';if(!is_readable($meta_file))return;$meta=json_decode(file_get_contents($meta_file),true);$pages=array('home'=>'Главная','projects'=>'Проекты','capital-remont'=>'Капитальный ремонт','design-remont'=>'Дизайнерский ремонт','remont-v-novostroyke'=>'Ремонт в новостройке','cosmetic-remont'=>'Косметический ремонт','remont-domov-pod-klyuch'=>'Ремонт домов под ключ','remont-kvartir-pod-klyuch'=>'Ремонт квартир под ключ','privacy'=>'Политика конфиденциальности','consent'=>'Согласие на обработку персональных данных');$front=0;
		...
		$thank=get_page_by_path('thank-you');if(!$thank){$id=wp_insert_post(array('post_type'=>'page','post_status'=>'publish','post_title'=>'Спасибо за заявку','post_name'=>'thank-you','post_content'=>'<main class="nexor-thank-you"><div><p class="text-primary text-6xl">✓</p><h1 class="heading-section">Заявка принята</h1><p class="text-muted-foreground">Инженер Nexor свяжется с вами в ближайшее время.</p><p><a href="/">Вернуться на главную</a></p><script>setTimeout(function(){location.href="/"},7000)</script></div></main>'));update_post_meta($id,'_nexor_noindex','1');}
		...
		if($front){update_option('show_on_front','page');update_option('page_on_front',$front);}update_option('permalink_structure','/%postname%/');
	}
...
if ( defined( 'WP_CLI' ) && WP_CLI ) {
	WP_CLI::add_command( 'nexor seed', static function(){ Nexor_Core::seed_content(); flush_rewrite_rules(); WP_CLI::success('Контент Nexor импортирован.'); } );
	WP_CLI::add_command( 'nexor migrate-media', static function(){ $result=Nexor_Core::migrate_media(); if(isset($result['error']))WP_CLI::error($result['error']); WP_CLI::log(wp_json_encode($result,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)); WP_CLI::success('Ссылки на изображения перенесены в Media Library.'); } );
	WP_CLI::add_command( 'nexor migrate-projects', static function(){ $result=Nexor_Core::migrate_projects(); if(isset($result['error']))WP_CLI::error($result['error']); WP_CLI::log(wp_json_encode($result,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)); WP_CLI::success('Данные и каталог проектов обновлены.'); } );
	WP_CLI::add_command( 'nexor enhancements-diagnostic', static function(){ WP_CLI::log(wp_json_encode(Nexor_Enhancements::diagnostics(),JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES)); WP_CLI::success('Диагностика выполнена без изменения данных.'); } );
}
