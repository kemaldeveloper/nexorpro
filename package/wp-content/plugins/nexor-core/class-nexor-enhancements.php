...
final class Nexor_Enhancements {
	...
	private static function seed_navigation(): void {
		...
		foreach ( array( 'primary'=>'Главное меню Nexor', 'mobile'=>'Мобильное меню Nexor' ) as $location => $name ) {
			...
			if ( ! $menu ) {
				...
				) as $item ) wp_update_nav_menu_item( $menu_id, 0, array( 'menu-item-title'=>$item[0], 'menu-item-url'=>$item[1], 'menu-item-status'=>'publish' ) );
			}
	...
	private static function merge_additional_rows( array $rows ): array {
		$by_id=array();foreach($rows as$row)if(!empty($row['id']))$by_id[sanitize_key($row['id'])]=$row;foreach(self::additional_seed()as$seed)if(!isset($by_id[$seed['id']]))$by_id[$seed['id']]=$seed;return array_values($by_id);
	}
	...
	public static function sanitize_prices( $input ): array {
		$default = self::defaults()[ self::PRICES ]; $input = is_array( $input ) ? $input : array();
	...
	public static function sanitize_video( $input ): array {
		$d=self::defaults()[self::VIDEO];$input=is_array($input)?$input:array();return array('enabled'=>empty($input['enabled'])?0:1,'heading'=>sanitize_text_field($input['heading']??$d['heading']),'text'=>sanitize_textarea_field($input['text']??''),'source_type'=>in_array(($input['source_type']??''),array('attachment','url'),true)?$input['source_type']:'url','attachment_id'=>absint($input['attachment_id']??0),'url'=>esc_url_raw($input['url']??''),'poster_id'=>absint($input['poster_id']??0),'transcript'=>sanitize_textarea_field($input['transcript']??''),'caption_attachment_id'=>absint($input['caption_attachment_id']??0));
	}
	public static function sanitize_additional( $input ): array {
		$d=self::defaults()[self::ADDITIONAL];$input=is_array($input)?$input:array();return array('enabled'=>empty($input['enabled'])?0:1,'heading'=>sanitize_text_field($input['heading']??$d['heading']),'intro'=>sanitize_textarea_field($input['intro']??$d['intro']),'rows'=>self::clean_rows((array)($input['rows']??array()),array('title'=>'text','subtitle'=>'text','description'=>'textarea','included_items'=>'textarea','benefit'=>'textarea','cta_label'=>'text','cta_mode'=>'text','cta_target'=>'url')));
	}
	public static function sanitize_promotions( $input ): array {
		$d=self::defaults()[self::PROMOTIONS];$input=is_array($input)?$input:array();$rows=self::clean_rows((array)($input['rows']??array()),array('title'=>'text','summary'=>'textarea','threshold_amount'=>'int','condition_text'=>'textarea','cta_label'=>'text','legal_text'=>'textarea'));return array('enabled'=>empty($input['enabled'])?0:1,'heading'=>sanitize_text_field($input['heading']??$d['heading']),'disclaimer'=>sanitize_textarea_field($input['disclaimer']??$d['disclaimer']),'featured_enabled'=>empty($input['featured_enabled'])?0:1,'featured_id'=>sanitize_key($input['featured_id']??$d['featured_id']),'featured_eyebrow'=>sanitize_text_field($input['featured_eyebrow']??$d['featured_eyebrow']),'featured_deadline'=>sanitize_text_field($input['featured_deadline']??$d['featured_deadline']),'rows'=>self::merge_seed_rows($rows));
	}
	public static function sanitize_budget( $input ): array {
		$d=self::defaults()[self::BUDGET];$input=is_array($input)?$input:array();return array('enabled'=>empty($input['enabled'])?0:1,'heading'=>sanitize_text_field($input['heading']??$d['heading']),'metric'=>sanitize_text_field($input['metric']??$d['metric']),'metric_label'=>sanitize_textarea_field($input['metric_label']??$d['metric_label']),'rows'=>self::clean_rows((array)($input['rows']??array()),array('title'=>'text','description'=>'textarea')));
	}
	public static function sanitize_timeline( $input ): array {
		$d=self::defaults()[self::TIMELINE];$input=is_array($input)?$input:array();return array('enabled'=>empty($input['enabled'])?0:1,'heading'=>sanitize_text_field($input['heading']??$d['heading']),'disclaimer'=>sanitize_textarea_field($input['disclaimer']??$d['disclaimer']),'rows'=>self::clean_rows((array)($input['rows']??array()),array('area'=>'text','new_build'=>'text','capital'=>'text','designer'=>'text')));
	}
	public static function sanitize_popup( $input ): array {
		$d=self::defaults()[self::POPUP];$input=is_array($input)?$input:array();return array('enabled'=>empty($input['enabled'])?0:1,'heading'=>sanitize_text_field($input['heading']??''),'body'=>sanitize_textarea_field($input['body']??''),'offer_text'=>sanitize_textarea_field($input['offer_text']??''),'cta_label'=>sanitize_text_field($input['cta_label']??$d['cta_label']),'minimum_delay_seconds'=>min(600,max(5,absint($input['minimum_delay_seconds']??15))),'suppression_days'=>min(365,max(1,absint($input['suppression_days']??7))),'storage_version'=>sanitize_key($input['storage_version']??'1'));
	}

	private static function option( string $name ): array { return wp_parse_args( (array) get_option( $name, array() ), self::defaults()[ $name ] ); }
	private static function enabled_rows( string $name, array $required ): array {
		$option=self::option($name);if(empty($option['enabled']))return array();$rows=array_filter((array)$option['rows'],static function($row)use($required){if(empty($row['enabled']))return false;foreach($required as$key)if(''===trim((string)($row[$key]??'')))return false;return true;});usort($rows,static fn($a,$b)=>intval($a['order']??0)<=>intval($b['order']??0));return array_values($rows);
	}

	public static function active_section_links(): array {
		$links=array();$prices=self::enabled_rows(self::PRICES,array('service_label','price_label','duration_label'));$price_option=self::option(self::PRICES);if($prices&&trim((string)$price_option['disclaimer']))$links[]=array('label'=>'Цены и сроки','url'=>home_url('/#prices'));
	...
	public static function frontend_config(): array {
		$popup=self::option(self::POPUP);$valid=!empty($popup['enabled'])&&trim($popup['heading'])&&trim($popup['body'])&&trim($popup['offer_text']);
		return array('searchUrl'=>home_url('/'),'exitIntent'=>$valid?$popup:array('enabled'=>0));
	}

	public static function render_admin_sections(): void {
		self::admin_styles();
		self::render_timeline_admin();self::render_budget_admin();self::render_prices_admin();self::render_video_admin();self::render_additional_admin();self::render_promotions_admin();self::render_popup_admin();self::admin_script();
	}
	private static function enabled_field(string $option,array $value):void{printf('<label><input type="checkbox" name="%s[enabled]" value="1" %s> Включить секцию</label>',esc_attr($option),checked(!empty($value['enabled']),true,false));}
	private static function text_field(string $option,string $key,string $label,array $value,string $type='text'):void{printf('<p><label><strong>%s</strong><br><input class="regular-text" type="%s" name="%s[%s]" value="%s"></label></p>',esc_html($label),esc_attr($type),esc_attr($option),esc_attr($key),esc_attr($value[$key]??''));}
	private static function textarea_field(string $option,string $key,string $label,array $value):void{printf('<p><label><strong>%s</strong><br><textarea class="large-text" rows="3" name="%s[%s]">%s</textarea></label></p>',esc_html($label),esc_attr($option),esc_attr($key),esc_textarea($value[$key]??''));}
	private static function render_rows(string $option,array $rows,array $fields):void{
		...
		foreach($fields as$key=>$field)echo '<th>'.esc_html($field[0]).'</th>';echo '<th>Действия</th></tr></thead><tbody>';
		$render=static function($row,$index)use($option,$fields){echo '<tr><td><input type="checkbox" name="'.esc_attr($option).'[rows]['.esc_attr($index).'][enabled]" value="1" '.checked(!empty($row['enabled']),true,false).'></td><td><input class="nexor-id" name="'.esc_attr($option).'[rows]['.esc_attr($index).'][id]" value="'.esc_attr($row['id']??'').'" placeholder="stable-id"><br><input class="small-text" type="number" name="'.esc_attr($option).'[rows]['.esc_attr($index).'][order]" value="'.esc_attr($row['order']??10).'"></td>';
			foreach($fields as$key=>$field){$tag=$field[1]??'text';echo '<td>';if('textarea'===$tag)echo '<textarea rows="3" name="'.esc_attr($option).'[rows]['.esc_attr($index).']['.esc_attr($key).']">'.esc_textarea($row[$key]??'').'</textarea>';else echo '<input type="'.esc_attr($tag).'" name="'.esc_attr($option).'[rows]['.esc_attr($index).']['.esc_attr($key).']" value="'.esc_attr($row[$key]??'').'">';echo '</td>';}
		...
		foreach($rows as$i=>$row)$render($row,$i);echo '</tbody></table><button type="button" class="button nexor-add">Добавить строку</button><template>';$render(array('id'=>'','enabled'=>0,'order'=>(count($rows)+1)*10),'__INDEX__');echo '</template></div>';
	}
	private static function render_prices_admin():void{$o=self::option(self::PRICES);echo '<section class="nexor-admin-section"><h2>Цены и сроки</h2>';self::enabled_field(self::PRICES,$o);self::text_field(self::PRICES,'heading','Заголовок',$o);self::textarea_field(self::PRICES,'intro','Вводный текст',$o);self::textarea_field(self::PRICES,'disclaimer','Дисклеймер',$o);self::render_rows(self::PRICES,(array)$o['rows'],array('service_page_id'=>array('ID страницы','number'),'service_label'=>array('Услуга','text'),'price_label'=>array('Цена','text'),'duration_label'=>array('Срок','text'),'note'=>array('Примечание','textarea'),'cta_label'=>array('CTA','text')));echo '</section>';}
	private static function render_video_admin():void{$o=self::option(self::VIDEO);echo '<section class="nexor-admin-section"><h2>Видео</h2>';self::enabled_field(self::VIDEO,$o);self::text_field(self::VIDEO,'heading','Заголовок',$o);self::textarea_field(self::VIDEO,'text','Описание',$o);self::text_field(self::VIDEO,'attachment_id','ID видео в медиатеке',$o,'number');self::text_field(self::VIDEO,'url','Разрешённый URL',$o,'url');self::text_field(self::VIDEO,'poster_id','ID постера',$o,'number');self::text_field(self::VIDEO,'caption_attachment_id','ID файла субтитров',$o,'number');self::textarea_field(self::VIDEO,'transcript','Транскрипт',$o);echo '</section>';}
	private static function render_additional_admin():void{$o=self::option(self::ADDITIONAL);echo '<section class="nexor-admin-section"><h2>Дополнительные услуги</h2>';self::enabled_field(self::ADDITIONAL,$o);self::text_field(self::ADDITIONAL,'heading','Заголовок',$o);self::textarea_field(self::ADDITIONAL,'intro','Описание под заголовком',$o);self::render_rows(self::ADDITIONAL,(array)$o['rows'],array('title'=>array('Название','text'),'subtitle'=>array('Подзаголовок','text'),'description'=>array('Описание','textarea'),'included_items'=>array('Что входит — по строке','textarea'),'benefit'=>array('Выгода','textarea'),'cta_label'=>array('CTA, необязательно','text'),'cta_mode'=>array('Режим CTA','text'),'cta_target'=>array('Цель CTA','text')));echo '</section>';}
	private static function render_budget_admin():void{$o=self::option(self::BUDGET);echo '<section class="nexor-admin-section"><h2>Как мы держим смету</h2>';self::enabled_field(self::BUDGET,$o);self::text_field(self::BUDGET,'heading','Заголовок',$o);self::text_field(self::BUDGET,'metric','Показатель',$o);self::textarea_field(self::BUDGET,'metric_label','Подпись к показателю',$o);self::render_rows(self::BUDGET,(array)$o['rows'],array('title'=>array('Заголовок','text'),'description'=>array('Описание','textarea')));echo '</section>';}
	private static function render_timeline_admin():void{$o=self::option(self::TIMELINE);echo '<section class="nexor-admin-section"><h2>Таблица сроков на главной</h2>';self::enabled_field(self::TIMELINE,$o);self::text_field(self::TIMELINE,'heading','Заголовок',$o);self::textarea_field(self::TIMELINE,'disclaimer','Текст под таблицей',$o);self::render_rows(self::TIMELINE,(array)$o['rows'],array('area'=>array('Площадь объекта','text'),'new_build'=>array('Новостройка','text'),'capital'=>array('Капитальный ремонт','text'),'designer'=>array('Дизайнерский ремонт','text')));echo '</section>';}
	private static function render_promotions_admin():void{$o=self::option(self::PROMOTIONS);echo '<section class="nexor-admin-section"><h2>Бонусы для клиентов</h2>';self::enabled_field(self::PROMOTIONS,$o);self::text_field(self::PROMOTIONS,'heading','Заголовок',$o);self::textarea_field(self::PROMOTIONS,'disclaimer','Общее условие',$o);printf('<label><input type="checkbox" name="%s[featured_enabled]" value="1" %s> Показывать отдельный временный баннер</label>',esc_attr(self::PROMOTIONS),checked(!empty($o['featured_enabled']),true,false));self::text_field(self::PROMOTIONS,'featured_id','ID бонуса для баннера',$o);self::text_field(self::PROMOTIONS,'featured_eyebrow','Надпись баннера',$o);self::text_field(self::PROMOTIONS,'featured_deadline','Дедлайн ISO 8601',$o);self::render_rows(self::PROMOTIONS,(array)$o['rows'],array('title'=>array('Название','text'),'summary'=>array('Кратко','textarea'),'threshold_amount'=>array('Порог, ₽','number'),'condition_text'=>array('Условия','textarea'),'cta_label'=>array('CTA','text'),'legal_text'=>array('Юридический текст','textarea')));echo '</section>';}
	private static function render_popup_admin():void{$o=self::option(self::POPUP);echo '<section class="nexor-admin-section"><h2>Отложенный popup</h2>';self::enabled_field(self::POPUP,$o);self::text_field(self::POPUP,'heading','Заголовок',$o);self::textarea_field(self::POPUP,'body','Текст',$o);self::textarea_field(self::POPUP,'offer_text','Предложение и условия',$o);self::text_field(self::POPUP,'cta_label','CTA',$o);self::text_field(self::POPUP,'minimum_delay_seconds','Задержка показа, сек.',$o,'number');self::text_field(self::POPUP,'suppression_days','Не показывать после закрытия, дней',$o,'number');self::text_field(self::POPUP,'storage_version','Версия ключа',$o);echo '</section>';}
	private static function admin_styles():void{echo '<style>.nexor-admin-section{margin:24px 0;padding:20px;background:#fff;border:1px solid #ccd0d4}.nexor-repeater{overflow:auto}.nexor-repeater table{min-width:1100px;margin:12px 0}.nexor-repeater td input:not([type=checkbox]),.nexor-repeater td textarea{width:100%}.nexor-repeater .nexor-id{min-width:150px}</style>';}
	private static function admin_script():void{echo '<script>document.addEventListener("click",function(e){const r=e.target.closest(".nexor-repeater");if(!r)return;const row=e.target.closest("tr");if(e.target.closest(".nexor-add")){const i=r.querySelectorAll("tbody tr").length,html=r.querySelector("template").innerHTML.replaceAll("__INDEX__",i);r.querySelector("tbody").insertAdjacentHTML("beforeend",html);const id=r.querySelector("tbody tr:last-child .nexor-id");id.value="item-"+Date.now().toString(36)+Math.random().toString(36).slice(2,7);}if(e.target.closest(".nexor-remove"))row.remove();if(e.target.closest(".nexor-up")&&row.previousElementSibling)row.parentNode.insertBefore(row,row.previousElementSibling);if(e.target.closest(".nexor-down")&&row.nextElementSibling)row.parentNode.insertBefore(row.nextElementSibling,row);});document.querySelector("form[action=options\\.php]")?.addEventListener("submit",function(){document.querySelectorAll(".nexor-repeater").forEach(r=>{const o=r.dataset.option;r.querySelectorAll("tbody tr").forEach((tr,i)=>tr.querySelectorAll("[name]").forEach(el=>el.name=el.name.replace(new RegExp(o+"\\[rows\\]\\[[^\\]]+\\]"),o+"[rows]["+i+"]")));});});</script>';}

	public static function add_service_meta_box():void{global $post;if(!$post instanceof WP_Post||!in_array($post->post_name,self::SERVICE_SLUGS,true))return;add_meta_box('nexor_service_details','Структура услуги и перелинковка',array(__CLASS__,'service_meta_box'),'page','normal','default');}
	public static function service_meta_box(WP_Post $post):void{if(!in_array($post->post_name,self::SERVICE_SLUGS,true))return;wp_nonce_field('nexor_service_meta','nexor_service_meta_nonce');$fields=array('summary'=>'Краткое описание','composition'=>'Состав работ (по строке)','price_label'=>'Цена / безопасный статус','duration_label'=>'Срок / безопасный статус','faq'=>'FAQ (вопрос и ответ по строке)','related_project_ids'=>'ID связанных проектов через запятую','related_content_ids'=>'ID связанных материалов через запятую','cta_label'=>'Текст CTA');foreach($fields as$key=>$label)printf('<p><label><strong>%s</strong><br><textarea name="nexor_service_%s" rows="%d" style="width:100%%">%s</textarea></label></p>',esc_html($label),esc_attr($key),in_array($key,array('composition','faq'),true)?4:2,esc_textarea(get_post_meta($post->ID,'_nexor_service_'.$key,true)));}
	public static function save_service_meta(int $post_id):void{if(!isset($_POST['nexor_service_meta_nonce'])||!wp_verify_nonce(sanitize_text_field(wp_unslash($_POST['nexor_service_meta_nonce'])),'nexor_service_meta')||!current_user_can('edit_post',$post_id)||wp_is_post_revision($post_id))return;$post=get_post($post_id);if(!$post||!in_array($post->post_name,self::SERVICE_SLUGS,true))return;foreach(array('summary','composition','price_label','duration_label','faq','related_project_ids','related_content_ids','cta_label')as$key)if(isset($_POST['nexor_service_'.$key]))update_post_meta($post_id,'_nexor_service_'.$key,sanitize_textarea_field(wp_unslash($_POST['nexor_service_'.$key])));}

	public static function search_policy(WP_Query $query):void{if(is_admin()||!$query->is_main_query()||!$query->is_search())return;$term=trim((string)$query->get('s'));$query->set('post_type',array('page','post','nexor_project'));$query->set('post_status','publish');$query->set('posts_per_page',10);if(''===$term)$query->set('post__in',array(0));}

	private static function insert_before(string $content,string $needle,string $html):string{$at=strpos($content,$needle);if(false===$at)return$content;$section=strrpos(substr($content,0,$at),'<section');if(false===$section)$section=$at;return substr($content,0,$section).$html.substr($content,$section);}
	private static function pull_section(string &$content,string $id):string{$pattern='/<section\b[^>]*\bid="'.preg_quote($id,'/').'"[^>]*>.*?<\/section>/s';if(!preg_match($pattern,$content,$match,PREG_OFFSET_CAPTURE))return'';$section=$match[0][0];$at=$match[0][1];$content=substr($content,0,$at).substr($content,$at+strlen($section));return$section;}
	...
	private static function timeline_section(): string {
		...
		foreach ( $rows as $row ) {
			$body .= sprintf(
				...
				esc_html( $row['area'] ), esc_html( $row['new_build'] ), esc_html( $row['capital'] ), esc_html( $row['designer'] )
			);
		}
	...
	private static function prices_section():string{$o=self::option(self::PRICES);$rows=self::enabled_rows(self::PRICES,array('service_label','price_label','duration_label'));if(!$rows||!trim($o['disclaimer']))return'';$body='';foreach($rows as$row){$url=absint($row['service_page_id']??0)&&'publish'===get_post_status(absint($row['service_page_id']))?get_permalink(absint($row['service_page_id'])):'';$service=$url?sprintf('<a href="%s">%s</a>',esc_url($url),esc_html($row['service_label'])):esc_html($row['service_label']);$body.=sprintf('<tr><th scope="row">%s</th><td>%s</td><td>%s</td><td>%s</td><td><button type="button" data-nexor-context-type="price" data-nexor-context-id="%s">%s</button></td></tr>',$service,esc_html($row['price_label']),esc_html($row['duration_label']),esc_html($row['note']??''),esc_attr($row['id']),esc_html($row['cta_label']?:'Уточнить'));}$intro=trim($o['intro'])?'<p>'.esc_html($o['intro']).'</p>':'';return'<section id="prices" class="nexor-enhancement-section"><div class="container-nexor"><h2 class="heading-section">'.esc_html($o['heading']).'</h2>'.$intro.'<div class="nexor-table-wrap"><table class="nexor-prices"><thead><tr><th>Услуга</th><th>Стоимость</th><th>Срок</th><th>Примечание</th><th></th></tr></thead><tbody>'.$body.'</tbody></table></div><p class="nexor-disclaimer">'.esc_html($o['disclaimer']).'</p></div></section>';}
	private static function budget_section(): string {
		...
		foreach ( $rows as $i => $row ) {
			$items .= sprintf(
				...
				0 === $i ? 'true' : 'false', $i + 1, esc_html( $row['title'] ), esc_html( $row['description'] )
			);
		}
	...
	private static function hero_promotion(): string {
		...
		foreach ( self::enabled_rows( self::PROMOTIONS, array( 'title', 'condition_text', 'cta_label', 'legal_text' ) ) as $row ) {
			if ( $featured_id === $row['id'] ) { $featured = $row; break; }
		}
		if ( ! $featured ) return '';
		return sprintf(
			...
			esc_attr( $o['featured_deadline'] ), esc_attr( $featured['title'] ), esc_html( $featured['title'] ), esc_attr( $featured['id'] ), esc_html( $featured['cta_label'] )
		);
	}
	private static function video_section():string{$o=self::option(self::VIDEO);if(empty($o['enabled']))return'';$src=$o['attachment_id']?wp_get_attachment_url(absint($o['attachment_id'])):esc_url_raw($o['url']);if(!$src)return'';$poster=$o['poster_id']?wp_get_attachment_image_url(absint($o['poster_id']),'large'):'';$track=$o['caption_attachment_id']?wp_get_attachment_url(absint($o['caption_attachment_id'])):'';$media=$o['attachment_id']?sprintf('<video controls preload="none" playsinline %s><source src="%s">%s</video>',$poster?'poster="'.esc_url($poster).'"':'',esc_url($src),$track?'<track kind="captions" srclang="ru" label="Русские субтитры" src="'.esc_url($track).'">':''):sprintf('<button type="button" class="nexor-video-facade" data-video-url="%s"%s><span>Воспроизвести видео</span></button>',esc_url($src),$poster?' style="background-image:url('.esc_url($poster).')"':'');return'<section id="video" class="nexor-enhancement-section"><div class="container-nexor"><h2 class="heading-section">'.esc_html($o['heading']).'</h2>'.(trim($o['text'])?'<p>'.esc_html($o['text']).'</p>':'').'<div class="nexor-video">'.$media.'</div>'.(trim($o['transcript'])?'<details><summary>Текстовая версия</summary><p>'.nl2br(esc_html($o['transcript'])).'</p></details>':'').'</div></section>';}
	private static function additional_section(): string {
		...
		foreach ( $rows as $i => $row ) {
			...
			$hotspots .= sprintf(
				...
				$i, esc_attr( $row['id'] ), 0 === $i ? 'true' : 'false', $i + 1, esc_html( $row['title'] )
			);
			$panels .= sprintf(
				...
				esc_attr( $row['id'] ), 0 === $i ? ' is-active' : '', $i, $i + 1, esc_html( $row['title'] ), esc_html( $row['subtitle'] ), esc_html( $row['description'] ), $items, esc_html( $row['benefit'] )
			);
		}
	...
	private static function service_details(int$post_id):string{$values=array();foreach(array('summary','composition','price_label','duration_label','faq','cta_label')as$key)$values[$key]=(string)get_post_meta($post_id,'_nexor_service_'.$key,true);$project_ids=array_filter(array_map('absint',explode(',',(string)get_post_meta($post_id,'_nexor_service_related_project_ids',true))));$content_ids=array_filter(array_map('absint',explode(',',(string)get_post_meta($post_id,'_nexor_service_related_content_ids',true))));$projects='';foreach(array_slice(array_unique(array_merge($project_ids,$content_ids)),0,6)as$id)if('publish'===get_post_status($id)&&in_array(get_post_type($id),array('nexor_project','post','page'),true))$projects.=sprintf('<li><a href="%s">%s</a></li>',esc_url(get_permalink($id)),esc_html(get_the_title($id)));if(!array_filter($values)&&!$projects)return'';$composition='';foreach(array_filter(array_map('trim',preg_split('/\R/u',$values['composition'])))as$line)$composition.='<li>'.esc_html($line).'</li>';$faq='';foreach(array_filter(array_map('trim',preg_split('/\R/u',$values['faq'])))as$line){$parts=array_map('trim',explode('|',$line,2));if(count($parts)===2)$faq.='<details><summary>'.esc_html($parts[0]).'</summary><p>'.esc_html($parts[1]).'</p></details>';}return'<section class="nexor-enhancement-section nexor-service-details"><div class="container-nexor"><h2 class="heading-section">Информация об услуге</h2>'.($values['summary']?'<p>'.esc_html($values['summary']).'</p>':'').($values['price_label']||$values['duration_label']?'<p><strong>'.esc_html($values['price_label']).'</strong> '.esc_html($values['duration_label']).'</p>':'').($composition?'<h3>Состав работ</h3><ul>'.$composition.'</ul>':'').($projects?'<h3>Связанные проекты и материалы</h3><ul>'.$projects.'</ul>':'').($faq?'<h3>Вопросы и ответы</h3>'.$faq:'').($values['cta_label']?'<button type="button" data-nexor-open-form="service">'.esc_html($values['cta_label']).'</button>':'').'</div></section>';}
	...
	private static function project_relations(int$project_id):string{$links='';foreach(self::SERVICE_SLUGS as$slug){$page=get_page_by_path($slug);if(!$page)continue;$ids=array_filter(array_map('absint',explode(',',(string)get_post_meta($page->ID,'_nexor_service_related_project_ids',true))));if(in_array($project_id,$ids,true))$links.=sprintf('<li><a href="%s">%s</a></li>',esc_url(get_permalink($page)),esc_html(get_the_title($page)));}return$links?'<section class="nexor-enhancement-section"><div class="container-nexor"><h2>Связанные услуги</h2><ul>'.$links.'</ul></div></section>':'';}
	public static function inject_frontend_content(string$content):string{
		...
		}elseif(is_page(self::SERVICE_SLUGS)){$content=self::service_page_shell($content);$block=self::service_details(get_queried_object_id());if($block)$content=preg_replace('/<\/main>/', $block.'</main>', $content,1);}elseif(is_singular('nexor_project')){$block=self::project_relations(get_queried_object_id());if($block)$content=preg_replace('/<\/main>/', $block.'</main>', $content,1);}return$content;
	}

	public static function resolve_lead_context(array$data):array|WP_Error{$map=array('additional_service_id'=>array(self::ADDITIONAL,'additional',array('title','description','benefit')),'promotion_id'=>array(self::PROMOTIONS,'promotion',array('title','condition_text','cta_label','legal_text')),'price_row_id'=>array(self::PRICES,'price',array('service_label','price_label','duration_label')));$resolved=array();foreach($map as$key=>$definition){$id=sanitize_key($data[$key]??'');if(!$id)continue;$found=null;foreach(self::enabled_rows($definition[0],$definition[2])as$row)if(hash_equals((string)$row['id'],$id)){$found=$row;break;}if(!$found)return new WP_Error('invalid_context','Выбранное предложение недоступно. Обновите страницу.',array('status'=>422));$snapshot=array('id'=>$id,'type'=>$definition[1],'title'=>$found['title']??$found['service_label'],'details'=>$found['condition_text']??($found['description']??($found['price_label'].'; '.$found['duration_label'])),'cta_label'=>$found['cta_label']??'','legal_text'=>$found['legal_text']??'','threshold_amount'=>absint($found['threshold_amount']??0),'timestamp'=>current_time('c'));$resolved[$key]=$id;$resolved[$definition[1].'_snapshot']=wp_json_encode($snapshot,JSON_UNESCAPED_UNICODE|JSON_UNESCAPED_SLASHES);}return$resolved;}
	public static function diagnostics():array{return array('schema_version'=>(string)get_option(self::VERSION_OPTION,''),'budget_enabled'=>''!==self::budget_section(),'prices_enabled'=>count(self::enabled_rows(self::PRICES,array('service_label','price_label','duration_label'))),'additional_enabled'=>count(self::enabled_rows(self::ADDITIONAL,array('title','description','included_items','benefit'))),'promotions_seeded'=>count((array)self::option(self::PROMOTIONS)['rows']),'promotions_enabled'=>count(self::enabled_rows(self::PROMOTIONS,array('title','condition_text','cta_label','legal_text'))),'video_valid'=>''!==self::video_section(),'popup_enabled'=>!empty(self::frontend_config()['exitIntent']['enabled']));}
}
