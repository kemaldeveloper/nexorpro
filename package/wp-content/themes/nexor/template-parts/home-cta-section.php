...
$lead = trim((string) ($args['lead'] ?? 'Инженер приедет на объект, оценит состояние помещения и зафиксирует все технические детали для дальнейшего планирования ремонта.'));
$button_label = trim((string) ($args['button_label'] ?? 'Записаться на замер'));
$note = trim((string) ($args['note'] ?? 'Консультация и выезд инженера — бесплатно и ни к чему не обязывают.'));
...
?>
<section class="py-24 md:py-32 bg-foreground">
  <div class="container-nexor">
    <div class="max-w-3xl mx-auto text-center">
      <h2 class="text-3xl sm:text-4xl md:text-5xl font-bold text-white mb-6"><?php echo esc_html($heading); ?></h2>
      <?php if ($lead) : ?>
      ...
      <div class="flex flex-col sm:flex-row gap-4 justify-center">
        ...
        <?php if ($phone_display && $phone_href !== 'tel:') : ?>
          <a href="<?php echo esc_url($phone_href); ?>" class="<?php echo esc_attr($phone_class); ?>">
      ...
      <?php if ($note) : ?>
        <p class="mt-8 text-base text-white/60"><?php echo esc_html($note); ?></p>
      <?php endif; ?>
    </div>
  </div>
</section>
