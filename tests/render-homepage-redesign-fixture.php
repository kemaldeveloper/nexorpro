...
{
  return 'https://nexorpro.ru' . $path;
}
function current_time($format)
{
  return '2026-07-30T20:00:00+03:00';
}
...
{
  return is_object($post) ? $post->post_title : 'Проект ' . $post;
}
function get_permalink($post)
{
  return 'https://nexorpro.ru/' . (is_object($post) ? $post->post_name : 'project-' . $post) . '/';
}
...
function nexor_render_home_faq_section(array $copy = array()): string
{
  ...
  $args = $copy;
  ob_start();
  include $theme_path . '/template-parts/home-faq-section.php';
  return (string) ob_get_clean();
}
...
function nexor_render_home_additional_section(array $copy = array()): string
{
  ...
  $args = $copy;
  ob_start();
  include $theme_path . '/template-parts/home-additional-section.php';
  return (string) ob_get_clean();
}
function nexor_render_home_nexor_system_section(array $copy = array()): string
{
  ...
  $args = $copy;
  ob_start();
  include $theme_path . '/template-parts/home-nexor-system-section.php';
  return (string) ob_get_clean();
}
...
$additional = array(
  ...
  array('id' => 'interior-design-project', 'enabled' => 1, 'order' => 20, 'title' => 'Дизайн-проект', 'subtitle' => 'Разработаем дизайн-проект для вашего ремонта', 'description' => 'Подготовим необходимую документацию для реализации без лишних вопросов на стройке.', 'included_items' => "Обмерный план.\nПланировочные решения.\nКомплект рабочих чертежей.\nРазвертки стен, пола и потолка.\n3D-визуализация будущего интерьера.", 'benefit' => 'Ремонт проходит без лишних переделок и неожиданностей.'),
  ...
  array('id' => 'designer-consultation', 'enabled' => 1, 'order' => 40, 'title' => 'Консультация дизайнера', 'subtitle' => 'Поможем создать интерьер, в котором будет комфортно жить', 'description' => 'Дизайнер поможет определиться со стилем, планировкой и цветовыми решениями.', 'included_items' => "Консультация по интерьеру.\nПодбор цветовых решений.\nПланировка и зонирование.\nРекомендации по освещению и эргономике.", 'benefit' => 'Получаете продуманные решения еще до начала ремонта.'),
  array('id' => 'photo-video-reports', 'enabled' => 1, 'order' => 50, 'title' => 'Фото- и видеоотчёты', 'subtitle' => 'Всегда знаете, что происходит на объекте', 'description' => 'Контролируйте ход ремонта дистанционно, даже если нет возможности приезжать на объект.', 'included_items' => "Регулярные фотоотчеты.\nВидео выполненных этапов.\nИнформация о ходе работ.\nКонтроль каждого этапа ремонта.", 'benefit' => 'Контролируете ремонт из любой точки, даже если не можете приехать на объект.'),
...
$GLOBALS['options'] = array(
  ...
  'nexor_additional_services' => array('enabled' => 1, 'heading' => 'Управляем ремонтом от идеи до вашего комфорта', 'intro' => 'Не ограничиваемся только ремонтом. При необходимости поможем с подбором материалов, дизайном, мебелью и другими вопросами, чтобы вам не пришлось искать отдельных специалистов.', 'rows' => $additional),
...
<head>
  ...
  <title>Nexor homepage redesign fixture</title>
  <link rel="stylesheet" href="<?php echo esc_attr($css_base); ?>">
  <link rel="stylesheet" href="<?php echo esc_attr($css_theme); ?>">
</head>
...
<body>
  ...
  ?>
  <script>
    window.NexorSettings = {
      restUrl: 'https://fixture.test/nexor/v1/',
      nonce: 'fixture',
      thankYou: '#thank-you',
      privacy: '#privacy',
      consent: '#consent',
    ...
    window.fetch = (url, options) => String(url).includes('/calculate') ? Promise.resolve(new Response(JSON.stringify({
      formatted: '2 350 000–2 950 000 ₽'
    }), {
      status: 200,
      headers: {
  ...
  <script src="<?php echo esc_attr($js_theme); ?>"></script>
</body>
