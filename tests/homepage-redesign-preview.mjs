...
if (production) { await evaluate(`document.querySelectorAll('.nexor-process-step')[1]?.click()`); await sleep(350); processDesktopInteractive = await evaluate(`(()=>{const items=[...document.querySelectorAll('.nexor-process-step')],details=items.map(x=>x.querySelector('h3 + div'));return{count:items.length,expanded:items.map(x=>x.getAttribute('aria-expanded')),secondVisible:getComputedStyle(details[1]).opacity==='1'&&details[1].getBoundingClientRect().height>0}})()`); files.push(await capture('36-process-production-desktop.png', 1440, 900, '.nexor-process-section')); }
...
if (production) files.push(await capture('37-process-production-mobile.png', 390, 844, '.nexor-process-section'));
const mobileAudit = await evaluate(`(()=>{const process=[...document.querySelectorAll('.nexor-process-step')];return{overflow:document.documentElement.scrollWidth>document.documentElement.clientWidth,viewport:document.documentElement.clientWidth,processSteps:process.length,allProcessDescriptionsVisible:process.every(item=>{const details=item.querySelector('h3 + div');return details&&getComputedStyle(details).opacity==='1'&&details.getBoundingClientRect().height>0})}})()`);
